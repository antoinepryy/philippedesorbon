...
class CRCAController extends Controller
{
    ...
    public function sendRequest(CartManager $cartManager, SessionInterface $session, LanguageManager $languageManager){
        ...
        $pbx_prenom_fact = $this->remove_accents($pbx_prenom_fact);	// Suppression des accents
        $pbx_nom_fact = $this->remove_accents($pbx_nom_fact);
        $pbx_adresse1_fact = $this->remove_accents($pbx_adresse1_fact);
        $pbx_city_fact = $this->remove_accents($pbx_city_fact);
